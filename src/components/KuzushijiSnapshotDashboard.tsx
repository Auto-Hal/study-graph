...
function DashboardView({ dashboard, cached, syncing, onSync }: { dashboard: SnapshotDashboard; cached: boolean; syncing: boolean; onSync: () => Promise<void> }) {
  ...
  return <main className="phase5-shell">
    ...
    <section className="phase5-page-heading"><div><p className="phase5-eyebrow">学習</p><h1 className="phase5-page-title">くずし字</h1><p className="phase5-context">{completedLectures > 0 ? `完了講義 ${completedLectures}件` : "講義を確認して始める"}</p></div><span className="phase5-row-status">{cached ? "端末の保存" : "同期済み"}</span></section>
