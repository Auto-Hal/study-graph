...
import { canonicalizeJson, type JsonValue } from "../exercises/revision.ts";
...
/**
 * Creates the immutable submission boundary. The hash delegates to the
 * existing Phase 4C helper and therefore excludes all client metadata.
 */
...
function asAttemptRequest(submission: OfflineSubmission): ExerciseAttemptRequest {
  return {
...
/** Hash authority remains exactly attemptId/instanceId/rawAnswer/grade/timing/hint. */
export function hashOfflineSubmission(submission: OfflineSubmission): string {
  assertValidOfflineSubmission(submission);
  return hashExerciseAttemptRequest(asAttemptRequest(submission));
}
...
export function confirmOfflineSubmission(
  ...
  return deepFreeze({
    ...
    submission,
    requestHash: hashOfflineSubmission(submission),
