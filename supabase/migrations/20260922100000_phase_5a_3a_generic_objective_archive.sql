...
-- Phase 5A-3a adds a project-neutral archive registration boundary.  The
-- existing Kuzushiji registrar remains unchanged for compatibility; this
-- function deliberately accepts any validated project/exercise identity.
create or replace function public.study_graph_register_objective_archive(
  p_release_id text,
...
begin
  if p_project_id is null or btrim(p_project_id) = ''
    or p_exercise_id is null or btrim(p_exercise_id) = ''
    or p_objective_id is null or btrim(p_objective_id) = ''
    or p_exercise_version is null or p_exercise_version <= 0 then
    raise exception using errcode = '22023', message = 'invalid_objective_archive';
  end if;
...
-- Resolve persisted immutable archive facts for any Objective project.  The
-- learner/instance pair is the complete ownership boundary; project and
-- exercise identity come only from the archived revision.
create or replace function public.study_graph_resolve_objective_instance_archive(
  p_instance_id uuid,
  p_learner_id uuid
)
returns table(
...
revoke all on function public.study_graph_register_objective_archive(
  text, integer, text, jsonb, text, text, text, integer, text, integer, text, jsonb, text
) from public, anon, authenticated, service_role;
grant execute on function public.study_graph_register_objective_archive(
  text, integer, text, jsonb, text, text, text, integer, text, integer, text, jsonb, text
) to service_role;

revoke all on function public.study_graph_resolve_objective_instance_archive(uuid, uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.study_graph_resolve_objective_instance_archive(uuid, uuid)
  to service_role;
