...
create table private.exercise_instances (
  ...
  presentation_hash text not null
    check (presentation_hash ~ '^[0-9a-f]{64}$'),
  renderer_version text,
  adapter_version text,
  locale text not null
    check (length(btrim(locale)) > 0),
  scope_evidence jsonb not null
    check (jsonb_typeof(scope_evidence) = 'object'),
  knowledge_binding jsonb,
  legacy_item_id text not null
    check (length(btrim(legacy_item_id)) > 0),
  legacy_item_kind text not null
    check (legacy_item_kind in ('character', 'mistake', 'knowledge')),
  legacy_exercise_id text not null
    check (length(btrim(legacy_exercise_id)) > 0 and length(legacy_exercise_id) <= 400),
  srs_target text not null
    check (srs_target = 'legacy-item'),
  srs_epoch text,
  issued_at timestamptz not null default now(),
  constraint exercise_instances_release_fk
    foreign key (release_id)
    references private.content_releases (release_id)
    on update restrict
    on delete restrict,
  constraint exercise_instances_revision_fk
    foreign key (revision_id)
    references private.exercise_revisions (revision_id)
    on update restrict
    on delete restrict,
  constraint exercise_instances_release_revision_fk
    foreign key (release_id, revision_id)
    references private.content_release_entries (release_id, revision_id)
    on update restrict
    on delete restrict
);

create index exercise_instances_learner_id_idx
  on private.exercise_instances (learner_id);
create index exercise_instances_revision_id_idx
  on private.exercise_instances (revision_id);

create table private.exercise_attempts (
  ...
  grading_authority text not null
    check (grading_authority = 'server'),
  grading_strategy_id text not null
    check (length(btrim(grading_strategy_id)) > 0),
  grading_strategy_version integer not null
    check (grading_strategy_version > 0),
  normalizer_version text not null
    check (length(btrim(normalizer_version)) > 0),
  is_correct boolean,
  self_evaluation text
    check (self_evaluation is null or self_evaluation in ('again', 'hard', 'good', 'easy')),
  effective_srs_grade text
    check (effective_srs_grade is null or effective_srs_grade in ('again', 'hard', 'good', 'easy')),
  response_ms integer
    check (response_ms is null or response_ms between 0 and 3600000),
  used_hint boolean not null default false,
  scope_accepted boolean not null,
  srs_applied boolean not null,
  srs_reason text not null
    check (srs_reason in ('applied', 'grader-unavailable', 'scope-not-eligible', 'revision-quarantined')),
  scheduler_version text,
  review_state_before jsonb,
  review_state_after jsonb,
  legacy_review_attempt_id bigint,
  receipt jsonb not null
    check (jsonb_typeof(receipt) = 'object'),
  submitted_at timestamptz not null default now(),
  constraint exercise_attempts_grading_result_check
    check (
      (grading_status = 'graded' and is_correct is not null)
      or (grading_status = 'ungraded' and is_correct is null)
    ),
  constraint exercise_attempts_srs_plan_check
    check (
      (srs_applied = true
        and srs_reason = 'applied'
        and scope_accepted = true
        and grading_status = 'graded'
        and effective_srs_grade is not null)
      or (srs_applied = false and srs_reason <> 'applied')
    ),
  constraint exercise_attempts_instance_fk
    foreign key (instance_id)
    references private.exercise_instances (instance_id)
    on update restrict
    on delete restrict,
  constraint exercise_attempts_legacy_review_attempt_fk
    foreign key (legacy_review_attempt_id)
    references public.review_attempts (id)
    on update restrict
    on delete restrict
);

-- Phase 4C-2 supplies this row guard. Reusing it keeps rollback additive:
-- removing 4C-3 does not remove a guard still owned by the archive tables.
create trigger exercise_instances_immutable
before update or delete on private.exercise_instances
for each row execute function private.study_graph_archive_row_is_immutable();

create trigger exercise_attempts_immutable
before update or delete on private.exercise_attempts
for each row execute function private.study_graph_archive_row_is_immutable();

alter table private.exercise_instances enable row level security;
alter table private.exercise_attempts enable row level security;

revoke all on table private.exercise_instances from public, anon, authenticated;
revoke all on table private.exercise_attempts from public, anon, authenticated;
